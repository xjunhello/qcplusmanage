...
$(document).ready(function () {
    ...
    $('#addform').validator({
		...
		valid : function(form) {
		    ...
		    $("#editor1").val(hotmessage.getData());
	        $.ajax({
	        	async: false,
	            type: "POST",
	        	...
	        	$("#submit_button").attr("disabled",false);
	        		if(data.success){
	        		    Lobibox.notify('info', {
		                 ...
		                 $('#addModal').modal('hide');
		                 setTimeout(function(){window.location='hot/list';}, 2000);
	        		}else{
...
<body>
	<div class="modal-header">
		...
		<h3 class="modal-title" id="myModalLabel">新增热点</h3>
	</div>
	<span id="error"></span>
	...
	<div class="modal-body">
		<div class="ibox-content">
			<form method="post" id="addform" action="hot/add"
				enctype="multipart/form-data" class="form-horizontal"  autocomplete="off">
				<div class="form-group">
					<label for="name"  > <span
						class="red-fonts">*&nbsp;&nbsp;&nbsp;</span>文章标题
					</label>
						<input id="name" name="title" placeholder="请输入"
							type="text" class="form-control" >
				</div>
				 
				  <div class="form-group">
				    	<span class="red-fonts">*&nbsp;&nbsp;&nbsp;</span><strong>文章内容</strong>
						 <textarea name="contentDesc" id="editor1" ></textarea>
						 <script>
				                var hotmessage = CKEDITOR.replace( 'editor1' );
				                CKFinder.setupCKEditor( hotmessage, '<%=basePath%>ckfinder/' ) ;
				            </script>
				  </div>
				<div class="form-group">
