...
<body>
	...
	<div id="wrapper">
		<jsp:include page="/view/public/menu.jsp">
			<jsp:param name="activeOne" value="commentlist"/>
		</jsp:include>
		<div id="page-wrapper" class="gray-bg">
			...
			<div class="row wrapper border-bottom white-bg page-heading">
				<div class="col-sm-10">
					...
					<ol class="breadcrumb">
						<li><a>其他</a></li>
						<li class="active"><strong>评价后台</strong></li>
					</ol>
				</div>
									...
									<table class="table table-striped table-bordered table-hover tablesorter" id="editable">
										...
										<tbody>
											<c:forEach items="${page.list}" var="comment">
												<tr class="gradeX">
													<td class="text-center"><input type="checkbox"name="checked" value="${comment.id}"></td>
													<td class="text-center" ><a href="#" class="link">${comment.orderCode }</a></td>
													<td class="text-center" ><a href="#" class="link" >${comment.loginname}</a></td>
													<td class="text-center" >${comment.descDate}</td>
													<td class="text-center" >
													<c:forEach var="x" begin="1" end="${comment.score }" step="1"> 
													<span class="fa fa-star"></span>
														</c:forEach> 
													<input type="hidden" id="comment_content_${comment.id}" value="${comment.content }">
													</td>
													<td class="text-center">
														 <a value="${comment.id }" class="btn btn-xs btn-white detailbtnclass">评价内容</a>
														 <a class="btn btn-xs btn-danger deletebtnclass" value="${comment.id }" > 删除</a>
													</td>
												</tr>
											</c:forEach>
										</tbody>
	...
	$(document).ready(function(){
		$(".detailbtnclass").on("click",function(){
			...
			$("#comment_content_body").html($("#comment_content_"+id).val());
			console.log($("#comment_content_"+id).val());
			$('#detailModal').modal();
